...
//    }
    @GetMapping("")
    public List<Um> getUm() throws JsonProcessingException {

        UriComponentsBuilder uriBuilder = UriComponentsBuilder.fromHttpUrl(serviceUrl + servicePath);
        ParameterizedTypeReference<List<Um>> responseType = new ParameterizedTypeReference<List<Um>>() { };
        ResponseEntity<List<Um>> response = restTemplate.exchange(uriBuilder.build().toUri(), HttpMethod.GET, null, responseType);
        return response.getBody();
    }
    @GetMapping("/{id}")
